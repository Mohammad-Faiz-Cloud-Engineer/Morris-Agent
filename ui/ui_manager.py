...
class UIManager:
    ...
    def __init__(
        ...
        self.width = width
        self.height = height
        self.assets_path = Path(assets_path) if assets_path else Path(__file__).resolve().parents[1] / "assets" / "face"
        self.fps = fps
    ...
    def _render_loop(self):
        ...
        # Use SDL's native desktop driver on Windows, macOS, and normal Linux
        # desktops.  The framebuffer mode remains available for a headless Pi,
        # but is intentionally never forced on desktop systems.
        if self.use_framebuffer and platform.system() == "Linux":
